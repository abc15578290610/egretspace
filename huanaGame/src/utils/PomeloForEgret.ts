...
module PomeloForEgret{
    export class Pomelo2{
        ...
        private onClose(e:egret.Event):void {
            ...
            this.emit(Pomelo2.EVENT_CLOSE,e);
        }

        private onIOError(e:egret.Event):void{
            this.emit(Pomelo2.EVENT_IO_ERROR, e);
            console.error('socket error: ', e);
        }

        private onKick(event){
            this.emit(Pomelo2.EVENT_KICK,event);
        }
        private onData(data){
            //probuff decode
        ...
        private heartbeatTimeoutCb(data) {
            ...
            if(gap > this.gapThreshold) {
                ...
                this.emit(Pomelo2.EVENT_HEART_BEAT_TIMEOUT,data);
                this._disconnect();
            }
        }
        public off(event?,fn?){
        ...
        private _disconnect():void {
            ...
            if (this.socket && this.socket.connected) this.socket.close();    //先暂时关闭
            this.socket = null;
            if(this.heartbeatId){
        ...
        private handshake(resData){
            ...
            if(data.code === this.RES_OLD_CLIENT) {
                this.emit(Pomelo2.EVENT_IO_ERROR, 'client version not fullfill');
                return;
            }

            if(data.code !== this.RES_OK) {
                this.emit(Pomelo2.EVENT_IO_ERROR, 'handshake fail');
                return;
            }
