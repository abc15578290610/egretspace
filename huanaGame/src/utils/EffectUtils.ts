...
module EffectUtils {
    ...
    /**
    * str             提示内容
    * effectType      动画类型 1：从上到下弹出 2：从左至右弹出 3：从底部弹出 4：从右到左 5：渐变 等等
    * isWarning       是否是警告，警告是红色
    */
    export function fadeIntEffect(obj:egret.DisplayObject,cartoonType:number = 1){
            if(obj.touchEnabled)obj.touchEnabled=true;
            var onComplete2:Function = function(){
                if(obj.touchEnabled)obj.touchEnabled=true;
            };
            let _x= obj.x;
            let _y= obj.y;
            if(cartoonType == 1){//top 
                obj.y=0-obj.height-100;
                egret.Tween.get(obj).to({y:_y},800,egret.Ease.elasticIn).call(onComplete2,this); 
            }else if(cartoonType == 2){//left
                obj.x=0-obj.width
                egret.Tween.get(obj).to({x:_x},1200,egret.Ease.cubicIn).call(onComplete2,this); 
            }else if(cartoonType == 3){//bottom
                obj.y=GameConfig.curHeight();
                egret.Tween.get(obj).to({y:GameConfig.curHeight()-obj.height},1000,egret.Ease.elasticIn).call(onComplete2,this); 
            }else if(cartoonType == 4){//right
                obj.x=GameConfig.curWidth()
                egret.Tween.get(obj).to({x:_x},1100,egret.Ease.cubicIn).call(onComplete2,this); 
            }else if(cartoonType == 5){
                obj.alpha=0;
                obj.x=GameConfig.curWidth()
                egret.Tween.get(obj).to({alpha:1,x:_x},800,egret.Ease.cubicIn).call(onComplete2,this); 
            }
    }   

    export function tipEffect(obj:egret.DisplayObject,cartoonType:number = 1){
            obj.alpha=0;
            var onComplete2:Function = function(){
            ...
            if(cartoonType == 1){//top 
                obj.y=0-obj.height;
                egret.Tween.get(obj).to({y:20,alpha:1},900).wait(1000).to({y:0-obj.height,alpha:0},900).call(onComplete2,this); 
            }else if(cartoonType == 2){//left
                obj.x=0-obj.width
                egret.Tween.get(obj).to({x:0},1600,egret.Ease.cubicIn).call(onComplete2,this); 
            }else if(cartoonType == 3){//bottom
                obj.y=GameConfig.curHeight()+obj.height;
                egret.Tween.get(obj).to({y:0},1000,egret.Ease.elasticIn).call(onComplete2,this); 
            }else if(cartoonType == 4){//right
                obj.x=GameConfig.curWidth()+obj.width;
                egret.Tween.get(obj).to({x:0},1500,egret.Ease.elasticIn).call(onComplete2,this); 
            }
    } 
    ...
    export function playEffect(obj,cartoonType:number = 1):void{
        ...
        var onComplete1:Function = function(){
            if(cartoonType == 1){
                ...
                egret.Tween.get(obj).to({scaleX:1,scaleY:1,x:obj.x - obj.width/4,y:obj.y - obj.height/4},500,egret.Ease.circOut).call(onComplete2,this); 
            }else if(cartoonType == 3){
                egret.Tween.get(obj).to({scaleX:1,scaleY:1,x:obj.x - obj.width/4,y:obj.y - obj.height/4},300).call(onComplete2,this); 
            }else if(cartoonType == 4){
                egret.Tween.get(obj).to({scaleX:1,scaleY:1,x:obj.x - obj.width/8,y:obj.y - obj.height/8},300,egret.Ease.elasticOut).call(onComplete2,this); 
            }
        };
        if(cartoonType == 4){
            egret.Tween.get(obj).to({scaleX:0.75,scaleY:0.75,x:obj.x + obj.width/8,y:obj.y + obj.height/8},100,egret.Ease.sineIn).call(onComplete1,this);                    
        }else{
    ...
    * interval      打字间隔 毫秒
    */    
    export function typerEffect(obj:eui.Label,content:string = "",interval:number = 200):void{
    ...
    /**闪动特效 */
    export function blinEffect(obj){
            return egret.Tween.get(obj,{loop:true}).to({alpha:0.5},200).to({alpha:1},200);
    }
}