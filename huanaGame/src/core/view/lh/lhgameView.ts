...
module game {
	export class lhgameView extends ViewElementUI {
		...
		private static _instance: lhgameView;
		public static getInstance(): lhgameView {
			if (!this._instance)
				this._instance = new lhgameView();
			return this._instance;
		}
		public constructor() {
			super();
			this.skinName = "lhgameViewSkin";
		}
		protected init() {
			super.init();
		...
		private curVideo=0
		private switchSence(){
			console.log(gameData.videoData)
			// playWithUrl()
			netTool.send(true);
		...
		private handleEvent(e: egret.TouchEvent) {
			...
			if(target==this.m_back){
				...
				EventManager.dispatchEventWith(EventNotify.CLOSE_GAME, false,{type:'lh'});
			}else if(target==this.m_sure||target==this.m_cf){//确定下注
				EffectUtils.playEffect(target,4);
				let arr = lhbetType.score;
				...
				if(data.bet.length>0){
					...
					rPomelo.request(gameData.lhroutes.doBet,data,this.doBetSuccess,this)
				}else{
		...
		private selectBetHandle(e: egret.TouchEvent){
			...
			let item = (((e.target as eui.Image).parent) as lhbetType);
			if(e.target.name){
				console.log(lhbetType.score)
		...
		public updataBetScore(bool=true){
			...
			var allscore = this.setTotalScore(lhbetType.score,lhbetType.onLine_score);
		...
		private doBetSuccess(res){
			if(res.code=='401'){
				...
				this.m_user.setGold(gameData.userInfo.ye);
				lhbetType.onLine_score = this.setTotalScore(lhbetType.score,lhbetType.onLine_score);
				lhbetType.reSetScore();
			}else{
				this.cleanGame();
			}
		...
		public getRecordBet(){
			...
			rPomelo.request(gameData.lhroutes.getRecord,data,this.doRecordBetSuccess,this)
		}
