module game {
	export class lhludan extends PanelUI{
		...
		public initData(str:any){
			...
			for(let k=0;k<this.data.length;k++){
				...
				lhstr +=this.data[k].slice(0,1);
			}
			this.lhstr = lhstr;
			let info = lhstr.split('').reduce((p, k) => (p[k]++ || (p[k] = 1), p), {})
		...
		//生成所有路图
		private setAllMap(lhstr){
			this.initZhuPan(lhstr)
			if(this.data[5]){
				var bjl = lhLib.getInstance();
				bjl.initNode(this.m_box,24,4);//大路
				bjl.init(lhstr)

				var bjl1 = lhLib.getInstance();
				bjl1.initNode(this.m_box1,12,5);//大眼路
				bjl1.init(bjl.getLuTu(1,bjl))

				var bjl2 = lhLib.getInstance();
				bjl2.initNode(this.m_box2,12,6);
				bjl2.init(lhLib.getInstance().getLuTu(2,bjl));

				var bjl3 = lhLib.getInstance();
				bjl3.initNode(this.m_box3,12,7);
				bjl3.init(lhLib.getInstance().getLuTu(3,bjl));
			}
			this.refectInfo();
		}
		private refectInfo(){
